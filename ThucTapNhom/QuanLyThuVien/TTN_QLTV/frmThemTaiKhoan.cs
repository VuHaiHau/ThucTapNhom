...
namespace TTN_QLTV
{
    public partial class frmThemTaiKhoan : Form
    {
        ...
        private void frmThemTaiKhoan_Load(object sender, EventArgs e)
        {
            ...
            acc.AutoComplete(tbx_QuyenHan, "SELECT phanquyen FROM Login");
            tbx_MaNV.SelectedIndex = -1;

            dgvThemTK.DataSource = acc.Select_Data("Select taikhoan, matkhau, TENNV, phanquyen from Login, NHANVIEN WHERE Login.MANV = NHANVIEN.MANV");
            ...
            dgvThemTK.Columns["TaiKhoan"].HeaderText = "Tài Khoản";
            dgvThemTK.Columns["MatKhau"].HeaderText = "Mật Khẩu";
            dgvThemTK.Columns["TENNV"].HeaderText = "Tên Nhân Viên";
            dgvThemTK.Columns["phanquyen"].HeaderText = "Quyền Hạn";
        ...
        private void bt_timkiem_Click(object sender, EventArgs e)
        {
            ...
            {
                dgvThemTK.DataSource = acc.Select_Data("Select taikhoan, matkhau, TENNV, phanquyen from Login, NHANVIEN WHERE Login.MANV = Login.MANV and (taikhoan like '%" + tbx_timkiem.Text + "%' OR matkhau like '%" + tbx_timkiem.Text + "%' OR TENNV like N'%" + tbx_timkiem.Text + "%' OR phanquyen like '%" + tbx_timkiem.Text + "%')");
                tbx_timkiem.Clear();
                dgvThemTK.ClearSelection();
            }
        }
        ...
        private void dgvThemTK_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                tbx_TDN.Text = dgvThemTK.CurrentRow.Cells["taikhoan"].Value.ToString().Trim();
                tbx_MK.Text = dgvThemTK.CurrentRow.Cells["matkhau"].Value.ToString().Trim();
                tbx_MaNV.Text = dgvThemTK.CurrentRow.Cells["TENNV"].Value.ToString().Trim();
                tbx_QuyenHan.Text = dgvThemTK.CurrentRow.Cells["phanquyen"].Value.ToString().Trim();
            }
        }
        ...
        private void btn_ghinhan_Click(object sender, EventArgs e)
        {
            if (key == 1)
            {
                ...
                else
                {
                    ...
                    dttk = acc.CheckSql("select * from Login where TaiKhoan ='" + tbx_TDN.Text + "'");
            ...
            if (key == 2)
            {
                ...
                else
                {
                    ...
                    else if (tbx_TDN.Text != dgvThemTK.CurrentRow.Cells["taikhoan"].Value.ToString().Trim())
                    {
                    ...
                    else
                    {
                        if (tbx_MaNV.Text == dgvThemTK.CurrentRow.Cells["TENNV"].Value.ToString() &&
                            tbx_MK.Text == dgvThemTK.CurrentRow.Cells["taikhoan"].Value.ToString().Trim() &&
                            tbx_TDN.Text == dgvThemTK.CurrentRow.Cells["matkhau"].Value.ToString().Trim() &&
                            tbx_QuyenHan.Text == dgvThemTK.CurrentRow.Cells["phanquyen"].Value.ToString().Trim())
                        {
            ...
            if (key == 3)
            {
                if (Username != dgvThemTK.CurrentRow.Cells["taikhoan"].Value.ToString().Trim())
                {
