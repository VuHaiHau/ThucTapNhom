...
namespace QuanLyKhoHang
{
    public partial class FrmBoPhan : Form
    {
        ...
        {
            if (MessageBox.Show("Bạn Chắc Chắn Muốn Hủy Không?", "Xác Nhận!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ClearText();
                btn_ghinhan.Enabled = true;
                btn_sua.Enabled = true;
        ...
        private void tbx_TenBP_TextChanged(object sender, KeyPressEventArgs e)
        {
            if (!char.IsControl(e.KeyChar) && char.IsDigit(e.KeyChar))
            {
                e.Handled = true;
        ...
        private void txb_dthoai(object sender, EventArgs e)
        {
